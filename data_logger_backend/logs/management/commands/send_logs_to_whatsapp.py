...
from logs.models import DeviceLog, AdminLog, AdminGroupInfo
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        while True:
            try: 
                self.send_unsent_logs()
            except Exception as e:
                self.stderr.write(self.style.ERROR(f"Error: {e}"))
            time.sleep(POLL_INTERVAL)

    def send_unsent_logs(self):
        active_groups = AdminGroupInfo.objects.filter(whatsapp_is_active=True)
        if not active_groups:
            self.stdout.write(self.style.WARNING("No active admin groups found."))
            return

        # استخدم ThreadPoolExecutor لإدارة الخيوط بشكل فعال
        with ThreadPoolExecutor(max_workers=5) as executor:
            for group_info in active_groups:
                ...
                if not all_logs:
                    self.stdout.write(self.style.SUCCESS(f"No unsent logs found for admin {group_info.admin.username}."))
                    continue
